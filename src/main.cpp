...
vector<int> splitStringToInt(const string& data, const string& delimiter) noexcept {
    vector<int> result;
    auto prepos = 0;
...
struct ParsingContext
{
    ...
    string serialize() noexcept {
        ...
        sort(vd.begin(), vd.end(), [](const unique_ptr<VersionDetail>& vd1, const unique_ptr<VersionDetail>& vd2){
            return splitStringToInt(vd1->tag, ".") > splitStringToInt(vd2->tag, ".");
        });
        for (const auto& i: vd) {
            result.append(i->tag);
            result.append("\n");
...
int main(int argc, char* argv[]) {
    auto file = fstream("/home/qile/playground/change_log_checker/data.txt");
    if (file.is_open()) {
        string line;
        ParsingContext ctx;
